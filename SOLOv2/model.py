...
def SOLOv2_head(ncls, filters_in, conv_filters=256, kernel_filters=256, head_layers=4, activation='gelu',
                name="SOLO_head", normalization='gn', normalization_kw={'groups': 32},
                block_params=None, **kwargs):
    ...
    class_head = layers.Conv2D(filters=ncls, kernel_size=3,
                               strides=1, padding='same', name=name + "_class_logits",
    ...
    kernel_head = layers.Conv2D(filters=kernel_filters, kernel_size=3,
                                strides=1, padding='same', name=name + "_pred_kernels",
    ...
    for level, fpn_feature in enumerate(fpn_features):
        ...
        for i in range(level+1):
            ...
            feature = convblock(filters_in=channels,
                                ...
                                normalization_kw=normalization_kw,
                                preact=False,
                                name='mask_head_P{}{}'.format(level+2, i+1))(feature)

        outputs.append(feature)

    seg_outputs = layers.Add()(outputs)
    seg_outputs = convblock(
        filters_in=mid_ch, filters_out=out_ch, kernel_size=1, activation=activation, normalization=normalization,
        normalization_kw=normalization_kw, preact=False, name='mask_head_output')(seg_outputs)
...
def SOLOv2(config):
    ...
    model = tf.keras.Model(
        inputs=backbone.input, outputs=[mask_output, feat_cls_list, feat_kernel_list],
...
def FPN(connection_layers,
    ...
    P5up = layers.UpSampling2D(
        size=(2, 2), interpolation=interpolation, name=name + "FPN_P5upsampled")(P5)
    ...
    if C2 is not None:
        P3up = layers.UpSampling2D(
            size=(2, 2), interpolation=interpolation, name=name + "FPN_P3upsampled")(P3)
        ...
        outputs.append(layers.Conv2D(pyramid_filters,
                                     kernel_size=3,
                                     padding='same',
                                     use_bias=True,
                                     strides=1,
    ...
    outputs.append(layers.Conv2D(pyramid_filters,
                                 ...
                                 strides=1,
                                 kernel_initializer=CONV_INIT,
                                 kernel_regularizer=l2(weight_decay), name=name + "FPN_P4_")(P4))
    ...
    outputs.append(layers.Conv2D(pyramid_filters,
                                 ...
                                 strides=1,
                                 kernel_initializer=CONV_INIT,
                                 kernel_regularizer=l2(weight_decay), name=name + "FPN_P5_")(P5))
    ...
    for i in range(extra_layers):
        x = layers.Conv2D(pyramid_filters,
                          ...
                          strides=1,
                          kernel_initializer=CONV_INIT,
                          kernel_regularizer=l2(weight_decay), name=name + "FPN_P{}_".format(i + 6))(outputs[-1])
        outputs.append(x)
...
def FPNv2(connection_layers,
    ...
    P5 = convblock(
        ...
        filters_out=pyramid_filters, kernel_size=(1, 1), preact=False,
        activation=activation, name=name + "FPN_C5P5_", normalization=normalization,
    ...
    C4P4 = convblock(
        ...
        filters_out=pyramid_filters, kernel_size=(1, 1), preact=False,
        activation=activation, name=name + "FPN_C4P4_", normalization=normalization,
        normalization_kw=normalization_kw)(C4)

    P4 = layers.Add(name=name + "FPN_P4add")([P5up, C4P4])

    P4up = layers.UpSampling2D(
        size=(2, 2), interpolation=interpolation, name=name + "FPN_P4upsampled")(P4)
    C3P3 = convblock(
        filters_in=input_shapes[2],
        filters_out=pyramid_filters, kernel_size=(1, 1), preact=False,
        activation=activation, name=name + "FPN_C3P3_", normalization=normalization,
    ...
    if C2 is not None:
        P3up = layers.UpSampling2D(
            size=(2, 2), interpolation=interpolation, name=name + "FPN_P3upsampled")(P3)
        P3up = convblock(filters_in=pyramid_filters, filters_out=pyramid_filters, kernel_size=(3, 3), preact=False,
                         activation=activation, name=name + "FPN_P3upconv_", normalization=normalization,
        ...
        C2P2 = convblock(
            ...
            name=name +
            "FPN_C2P2_",
            normalization=normalization, normalization_kw=normalization_kw)(C2)
        P2 = layers.Add(name=name + "FPN_P2add")([P3up, C2P2])
        ...
        outputs.append(
            convblock(
                ...
                activation=activation,
                name=name + "FPN_P2_",
                normalization=normalization, normalization_kw=normalization_kw,
    ...
    outputs.append(
        convblock(
            ...
            activation=activation,
            name=name + "FPN_P3_",
            normalization=normalization, normalization_kw=normalization_kw)(P3))
    ...
    outputs.append(
        convblock(
            ...
            activation=activation,
            name=name + "FPN_P4_",
            normalization=normalization, normalization_kw=normalization_kw)(P4))
    ...
    outputs.append(
        convblock(
            ...
            activation=activation,
            name=name + "FPN_P5_",
            normalization=normalization, normalization_kw=normalization_kw)(P5))

    for i in range(extra_layers):
        x = convblock(
            filters_in=pyramid_filters,
            filters_out=pyramid_filters, kernel_size=3, preact=False,
            activation=activation, strides=2, name=name + "FPN_P{}_".format(i + 6),
            normalization=normalization, normalization_kw=normalization_kw)(outputs[-1])
        outputs.append(x)
...
class SOLOv2Model(tf.keras.Model):
    ...
    def call(self, inputs, training=False, **kwargs):
        ...
        seg_preds, scores, cls_labels = compute_masks(flat_cls_pred,
                                                      flat_kernel_pred,
                                                      mask_head_output,
...
# def process_predictions(seg_preds, scores, cls_labels):

#     labeled_masks = tf.argmax(seg_preds + 1, axis=-1) + 1

def train(model, train_dataset, epochs, batch_size=1, val_dataset=None, steps_per_epoch=None, validation_steps=None,
          optimizer=None, callbacks=None, initial_epoch=0, prefetch=tf.data.AUTOTUNE, buffer=None):

    # Dataset returns name, image, mask, bboxes, classes, labels, obj_sizesname, image, mask, bboxes, classes, labels, obj_sizes

    if buffer is None:
        buffer = len(train_dataset)

    train_dataset = train_dataset.shuffle(
        buffer, reshuffle_each_iteration=True)
    train_dataset = train_dataset.apply(tf.data.experimental.dense_to_ragged_batch(batch_size))
    train_dataset = train_dataset.repeat(epochs)

    if val_dataset is not None and (validation_steps is None or validation_steps is None > 0):
        val_dataset = val_dataset.apply(tf.data.experimental.dense_to_ragged_batch(batch_size))
